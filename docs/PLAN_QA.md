...
- **Prueba con cliente real (14 de septiembre, OTP por SMS a un número de Nogales).** `solicitar-otp` y `verificar-otp` funcionan; la respuesta trae `{ token, clienteGuidId, nombre, correo, fotoPerfil }`. Con ese token: `POST /api/Solicitudes` 200 (SOL-00008/2026, guid `c7008a55-…`); `GET /api/Solicitudes/c7008a55-…` **404 "No se encontró la solicitud" para el propio dueño**; `enviar` **500**; la fila aparece en la bandeja del yonke con `guidId` `b9e4b3bc-…` y **`usuarioId: "Test"` aunque la creó un cliente real**; `POST /api/CotizacionYonke?solicitudYonkeGuidId=b9e4b3bc-…` **400 "La solicitud enviada al yonke no existe"**. Conclusión: el servidor guarda un usuario fijo "Test" en cada solicitud en lugar del usuario del token; eso explica el 404 del dueño y, muy probablemente, el rechazo al cotizar. Bloqueo de backend confirmado sin salvedades.
  - Ajuste en la app: el envío ya no consulta `GET /api/Solicitudes/{guid}` para confirmar la solicitud tras un 5xx de `enviar` (responde 404 incluso al dueño); basta con que la creación haya devuelto el guid en el mismo flujo.
